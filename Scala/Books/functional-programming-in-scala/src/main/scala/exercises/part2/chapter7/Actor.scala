...
/** Processes messages of type `A`, one at a time. Messages are submitted to the actor with the method `!`. Processing is performed asynchronously
  * using the provided executor.
  *
  * Memory consistency guarantee: when each message is processed by the `handler`, any memory that it mutates is guaranteed to be visible by the
  * `handler` when it processes the next message, even if the `executor` runs the invocations of `handler` on separate threads. This is achieved
  * because the `Actor` reads a volatile memory location before entering its event loop, and writes to the same location before suspending.
  *
