...
final case class NumberOfRowsEqualToValidation(
                                                limit: Long
                                              ) extends Validation with ValidationMetric with ValidationReport {

  override val name: String = "has_number_of_rows_equal_to"
  ...
  override def report(implicit observer: Observer): String = {
    ...
    if (totalRecords == limit)
      s"$emoji hasNumberOfRowsEqualTo($limit): total of records are the same of the specified limit!"
    else if (totalRecords > limit)
      s"$emoji hasNumberOfRowsEqualTo($limit): total of ${totalRecords - limit} records above the specified limit!"
    else
      s"$emoji hasNumberOfRowsEqualTo($limit): total of ${limit - totalRecords} records below the specified limit!"
  }
}
