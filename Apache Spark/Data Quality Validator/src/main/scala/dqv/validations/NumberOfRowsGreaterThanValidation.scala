...
final case class NumberOfRowsGreaterThanValidation(
                                                    limit: Long
                                                  ) extends Validation with ValidationMetric with ValidationReport {

  override val name: String = "has_number_of_rows_greater_than"
  ...
  override def report(implicit observer: Observer): String = {
    ...
    if (totalRecords > limit)
      s"$emoji hasNumberOfRowsGreaterThan($limit): total of ${totalRecords - limit} records above the specified limit!"
    else
      s"$emoji hasNumberOfRowsGreaterThan($limit): total of ${limit - totalRecords} records below the specified limit!"
  }
}
